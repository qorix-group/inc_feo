...
fn generate_ipc_events(
    ...
    for activity in activities.iter() {
        ...
        event_submap.insert(
            "init".to_string(),
            IpcEvent::new(&format!("{}_init", activity.lock().unwrap().getname())),
        );
        event_submap.insert(
            "init_ack".to_string(),
            IpcEvent::new(&format!("{}_init_ack", activity.lock().unwrap().getname())),
        );
        event_submap.insert(
            "step".to_string(),
            IpcEvent::new(&format!("{}_step", activity.lock().unwrap().getname())),
        );
        event_submap.insert(
            "step_ack".to_string(),
            IpcEvent::new(&format!("{}_step_ack", activity.lock().unwrap().getname())),
        );
        event_submap.insert(
            "term".to_string(),
            IpcEvent::new(&format!("{}_term", activity.lock().unwrap().getname())),
        );
        event_submap.insert(
            "term_ack".to_string(),
            IpcEvent::new(&format!("{}_term_ack", activity.lock().unwrap().getname())),
        );
    ...
    pub fn run(&self) {
        ...
        let pgminit = Program::new().with_action(
            Sequence::new()
                //step
                .with_step(self.init())
                .with_step(ForRange::new(10).with_body(self.step()))
                .with_step(self.terminate()),
        );
